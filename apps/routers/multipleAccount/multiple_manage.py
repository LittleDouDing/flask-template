...
from apps.validates.multiple_validate import AddMultipleAccountForm, DeleteMultipleAccountForm, \
    ModifyMultipleAccountForm, GetMultipleAccountForm
...
def search_topology():
    ...
    if form.validate():
        ...
        topology = MultipleManage(datadict=get_form_data(form), handle_type='search_multiple_account')
...
def modify_topology():
    ...
    if form.validate():
        topology = MultipleManage(datadict=get_form_data(form), handle_type='modify_multiple_account')
        result, code = handle_route(topology, del_redis_key='multiple')
        return jsonify(result), code
    return jsonify({'msg': get_error_message(form.errors), 'code': 403}), 403
...
def delete_topology():
    ...
    if form.validate():
        topology = MultipleManage(datadict=get_form_data(form), handle_type='delete_multiple_account')
        result, code = handle_route(topology, del_redis_key='multiple')
        return jsonify(result), code
    return jsonify({'msg': get_error_message(form.errors), 'code': 403}), 403
