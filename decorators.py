...
def permission_required(permission_name):
    def wrapper(fn):
        ...
        def decorator(*args, **kwargs):
            ...
            return {'msg': 'The current user does not have permission to perform this operation', 'code': 403}
    ...
    def wrapper(fn):
        ...
        def decorator(*args, **kwargs):
            ...
            if not claims.get('is_admin'):
                return {'msg': 'Only administrators can perform this operation', 'code': 403}
            return fn(*args, **kwargs)
...
def first_access_system():
    def wrapper(fn):
        @wraps(fn)
        def decorator(*args, **kwargs):
            this_access_week = asyncio.run(get_value('this_week_access'))
            if not this_access_week:
                add_new_access_week()
            return fn(*args, **kwargs)
    ...
    def wrapper(fn):
        ...
        def decorator(*args, **kwargs):
            ...
            if username != get_jwt_identity():
                return {'msg': 'Only the current user can perform this operation', 'code': 403}
            return fn(*args, **kwargs)
    ...
    def wrapper(fn):
        ...
        def decorator(*args, **kwargs):
            ...
            if not random_code or random_code.lower() != img_code.lower():
                return {'msg': 'The verification code is expired or incorrect', 'code': 403}
            return fn(*args, **kwargs)
        ...
        def decorator(*args, **kwargs):
            ...
            if usertype == 'admin':
                if not asyncio.run(get_value('random_code')):
                    return {'msg': "The email verification code has expired", 'code': 403}
                if email_code != asyncio.run(get_value('random_code')):
                    return {'msg': "The email verification code input error", 'code': 403}
            return fn(*args, **kwargs)
    ...
    def wrapper(fn):
        ...
        def decorator(*args, **kwargs):
            ...
            if not uploaded_file:
                return {'msg': 'Please upload the excel file with the suffix xls or xlsx', 'code': 403}
            ...
            if new_filename:
                ...
                if file_ext in current_app.config.get('UPLOAD_EXTENSIONS'):
                    return fn(*args, **kwargs)
                return {'msg': 'The file format does not conform to specification', 'code': 403}
...
def times_limited(limit_type):
    def wrapper(func):
        ...
        def decorator(*args, **kwargs):
            ...
            if not username:
                return {'msg': 'The username cannot be empty', 'code': 403}
            if limit_type == 'modify_info_num':
                ...
                if modify_info_num and int(modify_info_num) >= 10:
                    return {'msg': 'Too many modify information times, please try again in 3 hours', 'code': 403}
            if limit_type == 'change_pwd_num':
                ...
                if change_pwd_num and int(change_pwd_num) >= 10:
                    return {'msg': 'Too many change password times, please try again in 6 hours', 'code': 403}
            if limit_type == 'error_num':
                error_num = asyncio.run(get_value(username + '_error_num'))
                if error_num and int(error_num) >= 5:
                    return {'msg': 'Too many login errors, please try again in 30 minutes', 'code': 403}
            if limit_type == 'login_num':
                login_num = asyncio.run(get_value(username + '_login_num'))
                if login_num and int(login_num) >= 10:
                    return {'msg': 'Too many login times, please try again in 6 hours', 'code': 403}
